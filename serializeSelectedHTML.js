...
function Snapshooter(root) {
	...
	// list of shorthand properties based on CSSShorthands.in from the Chromium
	// code (https://code.google.com/p/chromium/codesearch)
	...
	/**
	 * Changes CSSStyleDeclaration to simple Object removing unwanted properties
	 * ('1','2','parentRule','cssText' etc.) in the process.
	 *
	...
	function styleDeclarationToSimpleObject(style) {
		...
		// Work around http://crbug.com/313670 (the "content" property is not
		// present as a computed style indexed property value).
		output.content = fixContentProperty(style.content);

		// Since shorthand properties are not available in the indexed array, copy
		// them from named properties
	...
	// Partial workaround for http://crbug.com/315028 (single words in the
	// "content" property are not wrapped with quotes)
...
// silliness with divs to get a single element to give to snapshooter
